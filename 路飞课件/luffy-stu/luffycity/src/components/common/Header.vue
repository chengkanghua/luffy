          ...
          <el-col class="nav" :span="10">
            <el-row>
                <el-col :span="3" v-for="(value,index) in nav_data_list" :key="value.id">
                  <router-link :to="value.link" :class="{active:nav_num===index+1}" @click="nav_num=index+1" v-if="!value.is_site">{{value.title}}</router-link>
    ...
    export default {
      ...
      data(){
        return {
          ...
          token:false,
          s_status:true,
          nav_num:0, // 用来切换导航栏的选中状态的
          list_status:false, //用来控制个人中心下拉菜单的动态显示，false不显示
          nav_data_list:[], // 存放导航数据的数组
        }
      },
      methods:{
        ulShowHandler(){
