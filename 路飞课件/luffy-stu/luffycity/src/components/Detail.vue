      ...
      <div class="main">
        <div class="course-info">
          ...
          <div class="wrap-right">
            ...
            <p class="data">{{course.students}}人在学&nbsp;&nbsp;&nbsp;&nbsp;课程总时长：{{course.lessons}}课时/{{course.lessons===course.pub_lessons?'更新完成':`已更新${course.pub_lessons}小时`}}&nbsp;&nbsp;&nbsp;&nbsp;难度：{{course.level_name}}</p>
            <div class="sale-time" v-if="course.discount_name">
              <p class="sale-type">{{course.discount_name}}</p>
              <p class="expire">距离结束：仅剩 {{course.active_time/60/60/24 | format_time}}天 {{course.active_time/60/60 % 24 | format_time}}小时 {{course.active_time/60 % 60 | format_time}}分 <span class="second">{{course.active_time % 60|format_time}}</span> 秒</p>
<!--              <p class="expire">距离结束：{{course.active_time}}秒</p>-->
          ...
          <div class="course-tab-list">
            ...
            <div class="tab-item" v-if="tabIndex==2">
              <div class="tab-item-title">
                ...
                <p class="chapter-length">共{{course_chapter.length}}章 {{course.lessons}}个课时</p>
              </div>
              <div class="chapter-item" v-for="(chapter,index) in course_chapter">
                <p class="chapter-title"><img src="/static/image/1.svg" alt="">第{{chapter.chapter}}章·{{chapter.name}}</p>
                <ul class="lesson-list">
                  <li class="lesson-item" v-for="(lesson,i) in chapter.coursesections">
                    <p class="name"><span class="index">{{chapter.chapter}}-{{lesson.lesson}}</span> 课程介绍-{{lesson.name}}<span class="free" v-if="lesson.free_trail">免费</span></p>
                    <p class="time">{{lesson.duration}} <img src="/static/image/chapter-player.svg"></p>
                    <button class="try" v-if="lesson.free_trail"><router-link :to="'/course/player/' + lesson.vid">立即试学</router-link></button>
    ...
    methods: {
      ...
      get_course_id(){
        ...
        if (course_id > 0){
          ...
          let self = this;
          this.$alert('参数有误！','别乱js填',{
            callback(){
              self.$router.go(-1);  //跳转到上一个页面
            }
          })
        }
      },
